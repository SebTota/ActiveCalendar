...
from backend.models import StravaCredentials, CalendarEvent, CalendarEventCreate, GoogleCalendarCredentials, \
    StravaNotificationAction, StravaNotification
from backend.models.calendar_template import CalendarTemplateType, CalendarTemplate
...
class StravaNotificationBackgroundTaskWorker:
    ...
    def __init__(self, notification: StravaNotification):
        self.notification: StravaNotification = notification
        self._db: Session = Session(engine)
    ...
    def _init_accessors(self):
        ...
        google_auth: Optional[GoogleCalendarCredentials] = crud.google_calendar_auth.get_for_user(self._db, self._user_id)
    ...
    def process(self):
        ...
        logger.info(f"Processing {len(templates)} templates for user {self._user_id}")
        ...
        if self.notification.action == StravaNotificationAction.create:
            for template_type, template in templates.items():
                self._process_summary_template(activity, template, template_type)

    def _process_activity_template(self, activity: Activity, activity_template: CalendarTemplate):
        ...
        if notification_action == StravaNotificationAction.create:
            ...
            crud.calendar_event.create_and_add_to_user(db=self._db, obj=cal_event_create)
