...
class CRUDCalendarEvent(CRUDBase[CalendarEvent, CalendarEventCreate, CalendarEventUpdate]):
    ...
    def get_by_start_and_end_date(self, db: Session, start_date: datetime, end_date: datetime) -> [CalendarEvent]:
        return db.query(CalendarEvent).filter(CalendarEvent.start_date == start_date,
                                              CalendarEvent.end_date == end_date).all()
